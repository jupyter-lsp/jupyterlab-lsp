...
class LSPKernelSpecInstallApp(Application):
    ...
    sys_prefix = Bool(default_value=True, help="Install into the current environment", config=True)
...
class LSPKernelSpecApp(Application):
    ...
    subcommands = dict(
        install=(LSPKernelSpecInstallApp, LSPKernelSpecInstallApp.description.splitlines()[0]),
    )
