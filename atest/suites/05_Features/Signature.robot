...
Triggers Signature Help After A Keystroke
    ...
    Wait Until Keyword Succeeds    40x    0.5s    Page Should Contain Element    ${SIGNATURE_BOX}
    Element Should Be Visible    ${SIGNATURE_BOX}
    Wait Until Keyword Succeeds    20x    0.5s    Element Should Contain    ${SIGNATURE_BOX}
    ...
    Wait Until Keyword Succeeds    40x    0.5s    Element Should Contain    ${SIGNATURE_HIGHLIGHTED_ARG}    y
    Press Keys    None    LEFT
    # should switch back to x
    Wait Until Keyword Succeeds    40x    0.5s    Element Should Contain    ${SIGNATURE_HIGHLIGHTED_ARG}    x
    # should close on closing bracket
    Press Keys    None    )
    Wait Until Keyword Succeeds    40x    0.5s    Page Should Not Contain Element    ${SIGNATURE_BOX}
...
Details Should Expand On Click
    ...
    Wait Until Keyword Succeeds    40x    0.5s    Page Should Contain Element    ${SIGNATURE_BOX}
    Element Should Be Visible    ${SIGNATURE_BOX}
    Wait Until Keyword Succeeds    20x    0.5s    Element Should Contain    ${SIGNATURE_BOX}    Short description.
